...
class _TFJobs(object):
    ...
    def create_single_node_placement(
        self, job, switch_id, node_id, num_gpu, num_cpu, mem=0
    ):
        """
    ...
    def move_to_runnable(self, job):
        ...
        job["executed_time"] = (
            0  # used for deciding priority queue, may be zeroed by last_pending_time
        )
        job["pending_time"] = 0
        job["last_pending_time"] = (
            0  # how much pending_time the job has since last entering the highest priority queue
        )
    ...
    def reserve_gpus(self, total_num):
        ...
        for job_dict in job_list:
            ...
            print(
                "%d-j, T%d, F%d, U%d, N%d, R%d; "
                % (
                    job_dict["num_gpu"],
                    job_dict["cur_gpu"],
                    job_dict["cur_free_gpu"],
                    job_dict["used_gpu"],
                    job_dict["demands"],
                    job_dict["reserve"],
                ),
