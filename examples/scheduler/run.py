...
flags.DEFINE_integer(
    "schedule_interval", 10, """The scheduling interval of scheduler, default is 10s"""
)
flags.DEFINE_string(
    "cluster_spec",
    None,
...
flags.DEFINE_boolean(
    "print", False, """Enable print out information, default is False"""
)
flags.DEFINE_boolean("flush_stdout", True, """Flush stdout, default is True""")
    ...
    while (len(JOBS.job_events) + len(JOBS.pending_jobs) + len(JOBS.running_jobs)) > 0:
        ...
        print(
            ...
            % (
                cur_time,
                len(JOBS.job_events),
                len(JOBS.pending_jobs),
                len(JOBS.running_jobs),
            )
        )
...
#done
def one_queue_fifo_sim_jobs(scheduler: Scheduler = None):
    """
    ...
    while (len(JOBS.job_events) + len(JOBS.pending_jobs)) > 0:
        while not scheduler._controller.done_queue.empty():
            finished_time, job_id, _, _, returncode = (
                scheduler._controller.done_queue.get()
            )
            ...
            if returncode == 0:
                ...
                scheduler._logger.info(
                    f"scheduler finishes {finished_job_cnt} jobs in all!"
                )
        ...
        assert (
            tmp_time - last_check_time > FLAGS.schedule_interval
            or tmp_time < FLAGS.schedule_interval
        )
        run_jobs = []
        for job in JOBS.pending_jobs:
            ...
            if ret:
                ...
                run_jobs.append(job)
                JOBS.pending_jobs.remove(job)
                scheduler._controller.execute(job)
        for job in run_jobs:
            job["status"] = "RUNNING"
            scheduler._logger.info(f"scheduler, {job['job_idx']}, run")
            print(tmp_time, "check: running ", job["job_idx"])
    ...
    while (len(JOBS.job_events) + len(JOBS.runnable_jobs)) > 0:
        ...
        if "start_jobs" in event:
            for s_job in event["start_jobs"]:
                ...
                s_job["remaining_gputime"] = int(
                    s_job["remaining_time"] * s_job["num_gpu"]
                )
                util.print_fn("---- job[%d] is added" % s_job["job_idx"])
        ...
        for rjob in JOBS.runnable_jobs:
            if "RUNNING" == rjob["status"]:
                ...
                rjob["remaining_time"] = int(
                    rjob["duration"] - rjob["total_executed_time"]
                )
                if gputime:
                    rjob["remaining_gputime"] = int(
                        rjob["remaining_time"] * rjob["num_gpu"]
                    )
...
# done
def shortest_first_jobs(scheduler: Scheduler = None, gputime=False):
    """
    new jobs are added to the end of the ending queue
    but in the queue, shortest (gpu) job first be served, until no resource
    """
    ...
    while (len(JOBS.job_events) + len(JOBS.runnable_jobs)) > 0:
        ...
        while scheduler.has_ready_jobs(scheduler.get_time()):
            ...
            for s_job in event["start_jobs"]:
                JOBS.move_to_runnable(s_job)
                s_job["remaining_time"] = (
                    s_job["iteration_time"] * s_job["remaining_iterations"]
                )
        ...
        if done_flag or new_flag:
            ...
            for rjob in JOBS.runnable_jobs:
                if "RUNNING" == rjob["status"]:
                    ...
                    rjob["remaining_time"] = (
                        rjob["iteration_time"] * rjob["remaining_iterations"]
                    )
                    if gputime:
                        rjob["remaining_gputime"] = (
                            rjob["remaining_time"] * rjob["num_gpu"]
                        )
                    print(
                        tmp_time,
            ...
            # sort jobs with shortest first
            if gputime:
                JOBS.runnable_jobs.sort(
                    key=lambda e: e.__getitem__("remaining_gputime")
                )
            else:
                JOBS.runnable_jobs.sort(key=lambda e: e.__getitem__("remaining_time"))
            ...
            for rjob in JOBS.runnable_jobs:
                if "RUNNING" == rjob["status"]:
                    ...
                    scheduler._controller.kill(rjob)
                ...
                if ret:
                    ...
                    if rjob["status"] == "PENDING":
                        run_jobs.append(rjob)
                    scheduler._controller.execute(rjob)
                else:
                    # rjob['status'] = 'PENDING'
                    if rjob["status"] == "RUNNING":
                        preempt_jobs.append(rjob)
...
def multi_dlas_sim_jobs(gputime=False, solve_starvation=0):
    ...
    while (
        len(JOBS.job_events)
        + int(sum(len(k.runnable_jobs) for k in JOBS.gpu_job.values()))
    ) > 0:
        if (len(JOBS.job_events) + len(end_events)) == 0:
            util.print_fn("This cluster is not large enough to run the job")
            break
        ...
        if "end_jobs" in event:
            for e_job in event["end_jobs"]:
                ...
                LOG.job_complete(e_job, event_time)
                util.print_fn(
                    "---- job[%d] %dG is completed"
                    % (e_job["job_idx"], e_job["num_gpu"])
                )
        ...
        for num_gpu, gjob in JOBS.gpu_job.items():
            if len(gjob.runnable_jobs) <= 0:
                continue
            print(
                "~~~~~ working on %d-GPU %djobs~~~~~"
                % (num_gpu, len(gjob.runnable_jobs)),
                end=" ",
            )
            ...
            for rjob in gjob.runnable_jobs:
                if "RUNNING" == rjob["status"]:
                    tmp = int(event_time - rjob["last_check_time"])
                    rjob["total_executed_time"] = int(rjob["total_executed_time"] + tmp)
                    rjob["executed_time"] = int(
                        rjob["executed_time"] + tmp
                    )  # decide job priority queue
                    rjob["last_check_time"] = event_time
                    ...
                    if cur_qid < int(gjob.num_queue - 1):  # not for the last queue
                        if j_gt >= gjob.queue_limit[cur_qid]:
                            ...
                            print(
                                "job %d demote to Q%d" % (rjob["job_idx"], rjob["q_id"])
                            )

                elif "PENDING" == rjob["status"]:
                    tmp = int(event_time - rjob["last_check_time"])
                    rjob["last_check_time"] = event_time
                    rjob["pending_time"] = int(
                        rjob["pending_time"] + tmp
                    )  # this is the total pending_time
                    if (
                        rjob["executed_time"] > 0
                    ):  # if not started yet, job is always in Q0 and no need to push_back
                        rjob["last_pending_time"] = int(
                    ...
                    if (
                        solve_starvation > 0
                        and rjob["q_id"] > 0
                        and rjob["total_executed_time"] > 0
                        and rjob["executed_time"] > 0
                    ):
                        if rjob["last_pending_time"] >= int(
                            rjob["executed_time"] * solve_starvation
                        ):
                            rjob["executed_time"] = 0
                ...
                if "RUNNING" == rjob["status"]:
                    ...
                    if qid < int(gjob.num_queue - 1):
                        if gputime:
                            jump_time = int(
                                math.ceil(
                                    (gjob.queue_limit[qid] - rjob["executed_time"])
                                    / rjob["num_gpu"]
                                )
                                + event_time
                            )
                    ...
                    "PENDING" == rjob["status"]
                ):  # when pending job will be push back to Q0
                    if (
                        solve_starvation > 0
                        and rjob["q_id"] > 0
                        and rjob["total_executed_time"]
                        and rjob["executed_time"] > 0
                    ):
                        diff_time = int(
                            rjob["executed_time"] * solve_starvation
                            - rjob["last_pending_time"]
                        )
                        if diff_time > 0:
...
def dlas_sim_jobs(scheduler: Scheduler, gputime=False, solve_starvation=0):
    """
    ...
    while (len(JOBS.job_events) + len(JOBS.runnable_jobs)) > 0:
        while (
            scheduler.get_time() - last_check_time < FLAGS.schedule_interval
            and scheduler.has_running_trainers(running_jobs)
        ):
        ...
        finished_jobs = []
        while not scheduler._controller.done_queue.empty():
            finished_time, job_id, worker_id, gpus, returncode = (
                scheduler._controller.done_queue.get()
            )
            e_job = JOBS.find_runnable_job(job_id)
        ...
        for rjob in JOBS.runnable_jobs:
            if "RUNNING" == rjob["status"]:
                if running_jobs > 0 and rjob["job_idx"] not in finished_jobs:
                    ...
                    scheduler._trainers.pop(rjob["job_idx"])
                    done_flag = True
                    scheduler._controller._logger.info(
                        f"controller, {rjob['job_idx']}, timeout"
                    )
        ...
        for rjob in JOBS.runnable_jobs:
            if "RUNNING" == rjob["status"]:
                ...
                tmp_error = (rjob["real_itertime"][0] - rjob["iteration_time"]) / rjob[
                    "iteration_time"
                ]
                JOBS.overhead_list[1].append(tmp_error)
                if rjob["last_finish_time"] < last_check_time:
                    finished_time = (
                        rjob["remaining_iterations"] * rjob["real_itertime"][0]
                    )
                else:
                    finished_time = (
                        rjob["last_finish_time"]
                        - rjob["last_check_time"]
                        + max(rjob["remaining_iterations"] - FLAGS.fast_forwarding, 0)
                        * rjob["real_itertime"][0]
                    )
                if finished_time <= tmp:
                    ...
                    if rjob["last_finish_time"] < last_check_time:
                        finished_iter = int(
                            (tmp_time - last_check_time) / rjob["real_itertime"][0]
                        )
                    else:
                        finished_iter = int(
                            FLAGS.fast_forwarding
                            + (tmp_time - rjob["last_finish_time"])
                            / rjob["real_itertime"][0]
                        )
                    rjob["remaining_iterations"] -= finished_iter
                    ...
                    cur_qid = rjob["q_id"]
                    while (
                        cur_qid < int(JOBS.num_queue - 1)
                        and j_gt >= JOBS.queue_limit[cur_qid]
                    ):
                        rjob["q_id"] = cur_qid + 1
        ...
        if tmp_n > 0:
            ...
            scheduler._logger.info(
                f"scheduler, error rate between sim and real: mean {tmp_mean}, std {tmp_std}"
            )
            ...
            if "RUNNING" == rjob["status"]:
                ...
                if qid < int(JOBS.num_queue - 1):
                    if gputime:
                        jump_time = int(
                            math.ceil(
                                (JOBS.queue_limit[qid] - rjob["executed_time"])
                                / rjob["num_gpu"]
                            )
                            + event_time
                        )
                        ...
                        jump_time = int(
                            JOBS.queue_limit[qid] - rjob["executed_time"] + event_time
                        )
                    if jump_time < next_job_jump:
                        next_job_jump = jump_time
...
def longest_pending_first_sim_jobs():
    ...
    while (len(JOBS.job_events) + len(JOBS.pending_jobs)) > 0:
        ...
        JOBS.pending_jobs.sort(
            key=lambda e: e.__getitem__("pending_time"), reverse=True
        )
        # JOBS.pending_jobs.sort(key = lambda e:e.__getitem__('num_gpu'))
